...
<head>
    ...
    <style>
        ...
        .mapel-list { display: flex; flex-wrap: wrap; gap: 8px; max-width: 420px; }
        .mapel-badge { background: #eef2f6; border: 1px solid #d0d8e4; border-radius: 6px; padding: 6px 10px; font-size: 13px; white-space: nowrap; }
        .mapel-badge strong { margin-left: 6px; }
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Nama Siswa</th>
                        <th>Kelas</th>
                        <th>Nilai per Mata Pelajaran</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @php($nilaiMapel = $nilaiPerSiswa[$rapor->Siswaid_siswa] ?? collect())
                            @if($nilaiMapel->isNotEmpty())
                                <div class="mapel-list">
                                    @foreach($nilaiMapel as $nilai)
                                        <span class="mapel-badge">{{ $nilai->mata_pelajaran }}:<strong>{{ $nilai->nilai_akhir }}</strong></span>
                                    @endforeach
                                </div>
                            @else
                                <span class="text-muted">Belum ada nilai mapel</span>
                            @endif
                        </td>
                    ...
                    @empty
                    <tr><td colspan="7" style="text-align: center;">Belum ada arsip rapor.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
