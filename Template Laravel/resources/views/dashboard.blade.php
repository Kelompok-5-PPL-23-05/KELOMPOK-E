    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .pill-input:checked + .pill-label {
            ...
            color: #fff;
            border-color: #6c8bbf;
            box-shadow: 0 4px 8px rgba(108,139,191,0.3);
        }
        ...
        .pill-input:checked + .pill-label:hover {
            background: #5a7aab;
        }
...
<body>
    ...
    <main class="main-content">
        ...
        <form id="filter-form" method="GET" action="{{ route('dashboard') }}">
            ...
            <div class="filter-row" style="flex-direction: column; gap: 20px;">
                {{-- Pilih Kelas --}}
                <div class="filter-group">
                    ...
                    <div class="pill-group">
                        @foreach($kelasList as $kelas)
                            <label>
                                <input type="radio" name="kelas_id" value="{{ $kelas->id_kelas }}" class="pill-input"
                                    onchange="document.getElementById('filter-form').submit()"
                                    {{ $selectedKelas == $kelas->id_kelas ? 'checked' : '' }}>
                                <span class="pill-label">{{ $kelas->nama_kelas }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="filter-group">
                    ...
                    <div class="pill-group">
                        @foreach($mataPelajaran as $mapel)
                            <label>
                                <input type="radio" name="mapel_id" value="{{ $mapel->id_mapel }}" class="pill-input"
                                    onchange="document.getElementById('filter-form').submit()"
                                    {{ $selectedMapel == $mapel->id_mapel ? 'checked' : '' }}>
                                <span class="pill-label">{{ $mapel->nama_mapel }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                        ...
                        <div class="input-row">
                            ...
                            <div class="input-group nilai">
                                ...
                                <input type="number" class="form-input" placeholder="1 - 100"
                                       name="nilai[{{ $loop->index }}][angka]" value="{{ old('nilai.'.$loop->index.'.angka') }}"
                                       min="1" max="100" oninput="batasNilai(this)" required>
                                @error('nilai.'.$loop->index.'.angka')
                                    <span style="color:#e53e3e; font-size:12px; margin-top:4px;">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="input-group catatan">
                                <label>Catatan</label>
                                <input type="text" class="form-input" placeholder="Catatan untuk siswa"
                                       name="nilai[{{ $loop->index }}][catatan]" value="{{ old('nilai.'.$loop->index.'.catatan') }}">
                                @error('nilai.'.$loop->index.'.catatan')
                                    <span style="color:#e53e3e; font-size:12px; margin-top:4px;">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
